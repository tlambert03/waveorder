...
class PycromanagerBackend(MMBackend):
    ...
    def connect(self) -> Any:
        ...
        try:
            ...
            # Check it works
            self._core.getAvailableConfigGroups()
        ...
        self._bridge._main_socket.send({"command": "connect", "debug": False})
        reply_json = self._bridge._main_socket.receive(timeout=500)
        zmq_mm_version = reply_json["version"]
        if zmq_mm_version != ZMQ_TARGET_VERSION:
            upgrade_str = (
                "upgrade"
                if version.parse(zmq_mm_version) < version.parse(ZMQ_TARGET_VERSION)
                else "downgrade"
            )
            logging.warning(
                (
    ...
    def generate_acquisition_settings(
        ...
        if channels:
            ...
            for channel in channels:
                try:
                    exposure = app.getChannelExposureTime(channel_group, channel)
                except:
                ...
                ss.channels.add(
                    app.getChannelSpec().fromJSONMetadata(str(channel_spec))
                )
