...
from waveorder.io.mm_backend import MMConnectionError, PycromanagerBackend
...
class TestPycromanagerBackend:
    ...
    def backend(self, mock_core, mock_studio, mock_bridge):
        ...
        with (
            patch("waveorder.io.mm_backend.Core") as mock_core_cls,
    ...
    def test_connect_failure(self):
        ...
        with patch("waveorder.io.mm_backend.Core") as mock_core_cls:
            ...
            backend = PycromanagerBackend()
            with pytest.raises(
                MMConnectionError, match="Failed to connect to Micro-Manager"
            ):
                backend.connect()
